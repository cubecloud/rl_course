class ConfigAgent:
    ...
    BATCH_SIZE = 8               # batch size (episodes)
    MASKED_ACTION = False
    GAMMA = 0.99                 # discount factor
    SYNC = 8                     # how often we use replay buffer (episodes) to update target network
    AGENTS_SYNC = 8              # how often to update the networks (weights) between running agents (episodes)
    EPS_START = 0.00
    EPS_END = 0.00
    EPS_DECAY = 1_000_000
    TAU = 1e-3                  # updating agents networks with this rate
