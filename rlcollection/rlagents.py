...
from rlcollection.rlnetworks import FCnet, EMBEDnet, ActorNet
...
class AgentMeta:
    ...
    def __init__(self, env, seed, device='cpu', net_model=FCnet, config=ConfigAgent):
        """
        Args:
    ...
    def save_movie_gif(self, gif_path_filename: str):
        frames, episode_reward, _ = self.get_frames_sequence()
        saveanimation(frames, gif_path_filename)
        return episode_reward, len(frames)

    def save_movie_mp4(self, mp4_path_filename: str):
        frames, episode_reward, _ = self.get_frames_sequence()
        save_mp4(frames, mp4_path_filename)
        return episode_reward, len(frames)
    ...
    def select_action(self, state, info, eps_threshold: Union[float or None] = None):
        ...
        if random.random() > eps_threshold:
            ...
            if ConfigAgent.MASKED_ACTION and "action_mask" in info.keys():
    ...
    def validation(self, validation_eps_num, validation_eps_count, ):
        ...
        for eps_num in range(0, val_eps_to_check):
            eps_frames, eps_reward, eps_done = self.get_frames_sequence(render=False)
            if eps_done and eps_reward >= self.ConfigAgent.REWARD_CONDITION:
                won_eps_count += 1
            eps_step_counts.append(len(eps_frames))
            eps_reward_counts.append(eps_reward)
...
class DQNAgent(AgentMeta):
    ...
    def __init__(self, env, seed, device='cpu', net_model=FCnet, config=ConfigAgent):
        ...
        Args:
            ...
            seed (int):     random seed
        """
        super().__init__(env, seed, device, net_model, config)
...
class A2CAgent(AgentMeta):
    ...
    def __init__(self, env, seed, device='cpu', net_model=ActorNet, config=ConfigAgent):
        """
        Args:
            env:            initialized environment class
            seed (int):     random seed
    ...
    def optimize_model(self, episode_data):
        ...
        for reward in episode_data.reward[::-1]:
            discounted_reward = reward + ConfigAgent.GAMMA * discounted_reward
