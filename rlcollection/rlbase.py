...
class RLBase:
    ...
    def __init__(self, env, agent=DQNAgent, agents_num=1, agents_devices: list = ('cpu',),
                 seed: int = 42, config=None, **kwargs):
        """
    ...
    def set_exp_id(self):
        setattr(self.ConfigAgent, 'EXP_ID', f'exp-{datetime.datetime.now(TZ).strftime("%d%m-%H%M%S")}')
...
class RLDQN(RLBase):
    ...
    def __init__(self, env, agent=DQNAgent, agents_num=1, agents_devices: list = ('cpu',), seed: int = 42,
                 config=None, **kwargs):
        """
